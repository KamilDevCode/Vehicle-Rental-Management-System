...
from services.json import JSONManager

from models.vehicles import Vehicle, TypeOfVehicle, VehicleStatus
...
class Menu:
    ...
    def add_vehicle(self):
        ...
        type = input("Enter your choice: ").upper()
        ...
        with Session as db:
            crud.add_vehicle(db, new_vehicle)
        print(f"Vehicle {brand} added successfully.")

    def rent_vehicle(self):
        vehicle_id = int(input("Enter vehicle ID to rent: "))
        vehicle = RentalService.check_vehicle_validity(vehicle_id)
        if vehicle and vehicle.status == VehicleStatus.AVAILABLE:
            RentalService.rent_vehicle(vehicle)
        else:
            print(f"Vehicle ID {vehicle_id} is not available.")
    ...
    def return_vehicle(self):
        ...
        if vehicle and vehicle.status == VehicleStatus.RESERVED:
            ...
            print(f"Vehicle ID {vehicle_id} is not reserved or not found.")

    def show_all_vehicles(self):
        vehicles = self.vehicle_db.get_all_vehicles()
        for vehicle in vehicles:
            print(vehicle)

    def show_available_vehicles(self):
        vehicles = self.vehicle_db.get_vehicles_by_status(VehicleStatus.AVAILABLE)
        for vehicle in vehicles:
            print(vehicle)

    def show_reserved_vehicles(self):
        vehicles = self.vehicle_db.get_vehicles_by_status(VehicleStatus.RESERVED)
        for vehicle in vehicles:
            print(vehicle)
